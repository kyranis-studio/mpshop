...
{if $is_order_position}
	...
	<script type="text/javascript">
		var come_from = '{$list_id|addslashes|escape:'html':'UTF-8'}';
...
{if !$simple_header}
	<script type="text/javascript">
		$(function() {
			$('table.{$list_id|escape:'html':'UTF-8'} .filter').keypress(function(e){
				var key = (e.keyCode ? e.keyCode : e.which);
				if (key == 13)
				{
					e.preventDefault();
					formSubmit(e, 'submitFilterButton{$list_id|escape:'html':'UTF-8'}');
				}
			})
			$('#submitFilterButton{$list_id|escape:'html':'UTF-8'}').click(function() {
				$('#submitFilter{$list_id|escape:'html':'UTF-8'}').val(1);
			});
...
<div class="alert alert-warning" id="{$list_id|escape:'html':'UTF-8'}-empty-filters-alert" style="display:none;">{l s='Please fill in at least one field to perform a search in this list.' mod='ets_delete_order'}</div>
{if isset($sql) && $sql}
	<form id="sql_form_{$list_id|escape:'html':'UTF-8'}" action="{$link->getAdminLink('AdminRequestSql', true)|escape:'html':'UTF-8'}&addrequest_sql=1" method="post" class="hide">
...
{block name="startForm"}
	<form method="post" action="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}{if isset($smarty.get.viewtrash)}&viewtrash=1{/if}" class="form-horizontal clearfix" id="form-{$list_id|escape:'html':'UTF-8'}">
{/block}
...
{if !$simple_header}
    ...
    {if isset($smarty.get.viewtrash) || isset($smarty.post.viewtrash)}
        <input type="hidden" name="filterviewtrash" value="1" />
    {/if}
	{block name="override_form_extra"}{/block}
	<div class="panel col-lg-12">
		<div class="panel-heading">
			{if isset($icon)}<i class="{$icon|escape:'html':'UTF-8'}"></i> {/if}{if is_array($title)}{$title|end|escape:'html':'UTF-8'}{else}{$title|escape:'html':'UTF-8'}{/if}
			{if isset($toolbar_btn) && count($toolbar_btn) >0}
				<span class="badge">{$list_total|escape:'html':'UTF-8'}</span>
				<span class="panel-heading-action">
                    {if !(isset($smarty.get.viewtrash) || isset($smarty.post.viewtrash))}
						<a id="desc-{$table|escape:'html':'UTF-8'}-viewtrash" class="list-toolbar-btn" href="{$link->getAdminLink('AdminOrders')|escape:'html':'UTF-8'}&viewtrash=1">
        					<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='View trash' mod='ets_delete_order'}" data-html="true" data-placement="top">
						...
						{foreach from=$toolbar_btn item=btn key=k}
        					{if $k != 'modules-list' && $k != 'back'}
        						<a id="desc-{$table|escape:'html':'UTF-8'}-{if isset($btn.imgclass)}{$btn.imgclass|escape:'html':'UTF-8'}{else}{$k|escape:'html':'UTF-8'}{/if}" class="list-toolbar-btn{if isset($btn.target) && $btn.target} _blank{/if}"{if isset($btn.href)} href="{$btn.href|escape:'html':'UTF-8'}"{/if}{if isset($btn.js) && $btn.js} onclick="{$btn.js|escape:'html':'UTF-8'}"{/if}>
        							<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{$btn.desc|escape:'html':'UTF-8'}" data-html="true" data-placement="top">
        				...
        				{/foreach}
        					<a class="list-toolbar-btn" href="javascript:location.reload();">
        						<span title="" data-toggle="tooltip" class="label-tooltip" data-original-title="{l s='Refresh list' mod='ets_delete_order'}" data-html="true" data-placement="top">
        				...
        				{if isset($sql) && $sql}
        					...
        					{if $sql_manager.view == 1}
        						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('.leadin').first().append('<div class=\'alert alert-info\'>' + $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val() + '</div>'); $(this).attr('onclick', '');">
        							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Show SQL query' mod='ets_delete_order'}" data-html="true" data-placement="top" >
        						...
        						<a class="list-toolbar-btn" href="javascript:void(0);" onclick="$('#sql_name_{$list_id|escape:'html':'UTF-8'}').val(createSqlQueryName()); $('#sql_query_{$list_id|escape:'html':'UTF-8'}').val($('#sql_query_{$list_id|escape:'html':'UTF-8'}').val().replace(/\s+limit\s+[0-9,\s]+$/ig, '').trim()); $('#sql_form_{$list_id|escape:'html':'UTF-8'}').submit();">
        							<span class="label-tooltip" data-toggle="tooltip" data-original-title="{l s='Export to SQL Manager' mod='ets_delete_order'}" data-html="true" data-placement="top" >
			...
			<script type="text/javascript">
				...
				$(function() {
					...
					if (btn_save.length > 0 && btn_submit.length > 0) {
						...
						lbl_save = $('#desc-{$table|escape:'html':'UTF-8'}-save div');
						...
						if (btn_save_and_stay.length > 0) {
							//get reference on current save link label
							lbl_save_and_stay = $('#desc-{$table|escape:'html':'UTF-8'}-save-and-stay div');
							//override save and stay link label with submit button value
							if (btn_submit.val().length > 0 && lbl_save_and_stay && !lbl_save_and_stay.hasClass('locked')) {
								lbl_save_and_stay.html(btn_submit.val() + " {l s='and stay' mod='ets_delete_order'} ");
							}
						}
						//hide standard submit button
						btn_submit.hide();
						//bind enter key press to validate form
						$('#{$table|escape:'html':'UTF-8'}_form').keypress(function (e) {
...
{elseif $simple_header}
	<div class="panel col-lg-12">
		{if isset($title)}<h3>{if isset($icon)}<i class="{$icon|escape:'html':'UTF-8'}"></i> {/if}{if is_array($title)}{$title|end|escape:'html':'UTF-8'}{else}{$title|escape:'html':'UTF-8'}{/if}</h3>{/if}
{/if}
					...
					<th class="{if isset($params.class)}{$params.class|escape:'html':'UTF-8'}{/if}{if isset($params.align)} {$params.align|escape:'html':'UTF-8'}{/if}">
						<span class="title_box{if isset($order_by) && ($key == $order_by)} active{/if}">
							...
							{if (!isset($params.orderby) || $params.orderby) && !$simple_header && $show_filters}
								<a {if isset($order_by) && ($key == $order_by) && ($order_way == 'DESC')}class="active"{/if} href="{$currentIndex|escape:'html':'UTF-8'}&amp;{$list_id|escape:'html':'UTF-8'}Orderby={$key|urlencode}&amp;{$list_id|escape:'html':'UTF-8'}Orderway=desc&amp;token={$token|escape:'html':'UTF-8'}{if isset($smarty.get.$identifier)}&amp;{$identifier|escape:'html':'UTF-8'}={$smarty.get.$identifier|intval}{/if}">
									<i class="icon-caret-down"></i>
								</a>
								<a {if isset($order_by) && ($key == $order_by) && ($order_way == 'ASC')}class="active"{/if} href="{$currentIndex|escape:'html':'UTF-8'}&amp;{$list_id|escape:'html':'UTF-8'}Orderby={$key|urlencode}&amp;{$list_id|escape:'html':'UTF-8'}Orderway=asc&amp;token={$token|escape:'html':'UTF-8'}{if isset($smarty.get.$identifier)}&amp;{$identifier|escape:'html':'UTF-8'}={$smarty.get.$identifier|intval}{/if}">
					...
					{if isset($multishop_active) && $multishop_active && $shop_link_type}
						<th>
							...
							{if $shop_link_type == 'shop'}
								{l s='Shop' mod='ets_delete_order'}
							{else}
								{l s='Shop group' mod='ets_delete_order'}
							{/if}
							</span>
						</th>
					{/if}
					...
					{foreach $fields_display AS $key => $params}
						<th {if isset($params.align)} class="{$params.align|escape:'html':'UTF-8'}" {/if}>
                          ...
                          name="{$list_id|escape:'html':'UTF-8'}Filter_{if isset($params.filter_key)}{$params.filter_key|escape:'html':'UTF-8'}{else}{$key|escape:'html':'UTF-8'}{/if}">
										<option value="">-</option>
										<option value="1" {if $params.value == 1} selected="selected" {/if}>{l s='Yes' mod='ets_delete_order'}</option>
										<option value="0" {if $params.value == 0 && $params.value != ''} selected="selected" {/if}>{l s='No' mod='ets_delete_order'}</option>
									</select>
								{elseif $params.type == 'date' || $params.type == 'datetime'}
									<div class="date_range row">
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'html':'UTF-8'}_0" name="local_{$params.name_date|escape:'html':'UTF-8'}[0]"  placeholder="{l s='From' mod='ets_delete_order'}" />
 										...
 										<div class="input-group fixed-width-md center">
											<input type="text" class="filter datepicker date-input form-control" id="local_{$params.id_date|escape:'html':'UTF-8'}_1" name="local_{$params.name_date|escape:'html':'UTF-8'}[1]"  placeholder="{l s='To' mod='ets_delete_order'}" />
								...
								{elseif $params.type == 'select'}
									{if isset($params.filter_key)}
										<select class="filter{if isset($params.align) && $params.align == 'center'}center{/if}" onchange="$('#submitFilterButton{$list_id|escape:'html':'UTF-8'}').focus();$('#submitFilterButton{$list_id|escape:'html':'UTF-8'}').click();" name="{$list_id|escape:'html':'UTF-8'}Filter_{$params.filter_key|escape:'html':'UTF-8'}" {if isset($params.width)} style="width:{$params.width|escape:'html':'UTF-8'}px"{/if}>
									...
									{/if}
								{else}
									<input type="text" class="filter" name="{$list_id|escape:'html':'UTF-8'}Filter_{if isset($params.filter_key)}{$params.filter_key|escape:'html':'UTF-8'}{else}{$key|escape:'html':'UTF-8'}{/if}" value="{$params.value|escape:'html':'UTF-8'}" {if isset($params.width) && $params.width != 'auto'} style="width:{$params.width|escape:'html':'UTF-8'}px"{/if} />
								{/if}
							{/if}
						</th>
					{/foreach}
						...
						<th class="actions">
							...
							<span class="pull-right">
								...
								<button type="submit" id="submitFilterButton{$list_id|escape:'html':'UTF-8'}" name="submitFilter" class="btn btn-default" data-list-id="{$list_id|escape:'html':'UTF-8'}">
									<i class="icon-search"></i> {l s='Search' mod='ets_delete_order'}
								</button>
								{if $filters_has_value}
									<button type="submit" name="submitReset{$list_id|escape:'html':'UTF-8'}" class="btn btn-warning">
										<i class="icon-eraser"></i> {l s='Reset' mod='ets_delete_order'}
									</button>
								{/if}
							</span>
							{/if}
