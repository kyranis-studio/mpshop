    ...
    public function setMedia()
    {
        ...
        foreach (glob(_PS_ROOT_DIR_ . '/modules/' . $this->name . "/views/css/*.css") as $filename) {
            $this->context->controller->registerStylesheet('modules-thecheckout-' . ($i++),
                ...
                array('media' => 'all', 'priority' => ('front.less.css' == Tools::substr($filename, -14)) ? 1150 : 1200));
        }
...
//        ['position' => 'bottom', 'priority' => 140, 'server' => 'remote']);
    ...
    private function generateFormFields($fields, $addressData, $isInvoiceAddress)
    {
        ...
        $autoCompleteAttributes = array(
            'dni'       => 'dni', // though, this is not valid attribute value, we need to prevent filling in anything else
...
//            (null == trim($formFieldsInvoiceMapped['vat_number']['value']));
    ...
    public function parentInitContent()
    {
        ...
        if (class_exists('AmazonPayHelper') && method_exists('AmazonPayHelper','isAmazonPayCheckout') && AmazonPayHelper::isAmazonPayCheckout()) {
    ...
    public function initContent()
    {
        ...
        if (Tools::getValue('ajax_request')) {
            ...
            if (!in_array($action,
                array('modifyAccountAndAddress', 'modifyAddressSelection', 'modifyAccount', 'updateQuantity', 'deleteFromCart'))) {
...
//    }
    ...
    private function getShippingOptionsBlock()
    {
        ...
        if (isset($opc_form_radios['delivery_option'])) {
            $this->context->smarty->assign('customerSelectedDeliveryOption', reset($opc_form_radios['delivery_option']));
        }
        $this->context->smarty->assign('forceToChooseCarrier', (bool)$this->module->config->force_customer_to_choose_carrier);
...
//        }
    ...
    private function silentRegistration($email)
    {
        ...
        $this->modifyAccount(array("email" => $email), "a", "a", true, false);
    }
