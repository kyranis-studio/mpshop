...
class Ets_megamenu extends Module
{
    ...
    public function __construct()
    {
        ...
        self::$menus = array(
            'form' => array(
                'legend' => array(
                    'title' => (int)$itemId ? $this->l('Edit menu') : $this->l('Add menu'),
                ),
                'input' => array(),
                'submit' => array(
                    'title' => $this->l('Save'),
        ...
        self::$tab_class = array(
            'form' => array(
                'legend' => array(
                    'title' => (int)$itemId ? $this->l('Edit tab') : $this->l('Add tab'),
                ),
                'input' => array(),
                'submit' => array(
                    'title' => $this->l('Save'),
        ...
        self::$columns = array(
            'form' => array(
                'legend' => array(
                    'title' => (int)$itemId ? $this->l('Edit column') : $this->l('Add column'),
                ),
                'input' => array(),
                'submit' => array(
                    'title' => $this->l('Save'),
        ...
        self::$blocks = array(
            'form' => array(
                'legend' => array(
                    'title' => (int)$itemId ? $this->l('Edit block') : $this->l('Add block'),
                ),
                'input' => array(),
                'submit' => array(
                    'title' => $this->l('Save'),
        ...
        if($menus)
            foreach($menus as &$menu)
            {
                ...
                if($menu['menu_img_link'])
                    $menu['menu_img_link']=$this->context->link->getMediaLink(_PS_ETS_MM_IMG_.$menu['menu_img_link'])  ;
                if($menu['background_image'])
                    $menu['background_image']=$this->context->link->getMediaLink(_PS_ETS_MM_IMG_.$menu['background_image']);
    ...
    public function getTabs($id_menu = false, $id_tab = false, $id_lang = false)
    {
        ...
        if($tabs)
            foreach($tabs as &$tab)
            {
                ...
                if($tab['tab_img_link'])
                    $tab['tab_img_link']= $this->context->link->getMediaLink(_PS_ETS_MM_IMG_.$tab['tab_img_link'])  ;
                if($tab['background_image'])
                    $tab['background_image']=$this->context->link->getMediaLink(_PS_ETS_MM_IMG_.$tab['background_image']) ;
                $tab['url'] = $this->getTabLink($tab);
                if($this->context->language->is_rtl)
                {
    ...
    public function getTabLink($tab)
    {
        if(isset($tab['link_type']))
        {
            switch($tab['link_type'])
            {
                case 'CUSTOM':
                    return $tab['url'];
                case 'CMS':
                    return $this->context->link->getCMSLink((int)$tab['id_cms']);
                case 'CATEGORY':
                    return $this->context->link->getCategoryLink((int)$tab['id_category']);
                case 'MNFT':
                    $manufacturer = new Manufacturer((int)$tab['id_manufacturer'], (int)$this->context->language->id);
                    if(!is_null($manufacturer->id))
                    {
                        ...
                        return $this->context->link->getManufacturerLink((int)$tab['id_manufacturer'], $manufacturer->link_rewrite);
                    }
                    return '#';
                case 'MNSP':
                    $supplier= new Supplier((int)$tab['id_supplier'],(int)$this->context->language->id);
    ...
    public function translates()
    {
        self::$trans = array(
            ...
            'file_too_large' => sprintf($this->l('Upload file cannot be larger than %sMB'),Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE')),
            ...
            'file_valid_type'=> $this->l('File is not in the correct format, accepted formats: jpg, gif, jpeg, png.'),
    ...
    private function processImport($zipfile = false)
    {
        ...
        if(!$errors)
        {
            $this->copy_directory(dirname(__FILE__).'/views/img/upload',_PS_ETS_MM_IMG_DIR_);
            $this->rrmdir(dirname(__FILE__).'/views/img/upload');
    ...
    {
        $sqls = array();
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_block` ADD INDEX IF NOT EXISTS (`id_column`, `sort_order`, `enabled`, `display_mnu_img`, `display_mnu_name`, `display_suppliers_img`, `display_suppliers_name`,`product_count`, `display_title`, `show_description`, `show_clock`) ';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_block_lang` ADD PRIMARY KEY IF NOT EXISTS (`id_block`, `id_lang`)';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_column` ADD INDEX IF NOT EXISTS (`id_menu`, `id_tab`, `is_breaker`, `sort_order`) ';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_menu` ADD INDEX IF NOT EXISTS (`sort_order`, `enabled`, `enabled_vertical`, `menu_open_new_tab`, `id_cms`, `id_manufacturer`, `id_supplier`, `id_category`, `menu_ver_alway_show`, `menu_ver_alway_open_first`, `menu_ver_hidden_border`, `display_tabs_in_full_width`)';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_menu_lang` ADD PRIMARY KEY IF NOT EXISTS (`id_menu`, `id_lang`)';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_menu_shop` ADD PRIMARY KEY IF NOT EXISTS (`id_menu`, `id_shop`)';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_tab` ADD UNIQUE IF NOT EXISTS (`id_menu`, `enabled`, `tab_sub_content_pos`, `id_category`, `id_cms`, `id_manufacturer`, `id_supplier`, `sort_order`)';
        $sqls[] = 'ALTER TABLE `'._DB_PREFIX_.'ets_mm_tab_lang` ADD PRIMARY KEY IF NOT EXISTS (`id_tab`, `id_lang`)';
        foreach($sqls as $sql)
