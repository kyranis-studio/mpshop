...
use TrustedShopsIntegration;

class TrustedShopsIntegrationWrapper extends TrustedShopsIntegration
{
    public function getTrustedShopId()
    {
        $tsConfig = $this->getTSConfig();
        if ($tsConfig and array_key_exists('id_trusted_shops', $tsConfig)) {
            return $tsConfig['id_trusted_shops'];
        }
